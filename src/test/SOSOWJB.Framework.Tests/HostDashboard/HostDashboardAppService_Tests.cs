...
using SOSOWJB.Framework.MultiTenancy.HostDashboard;
using SOSOWJB.Framework.MultiTenancy.HostDashboard.Dto;
using SOSOWJB.Framework.MultiTenancy.Payments;

namespace SOSOWJB.Framework.Tests.HostDashboard
{
    public class HostDashboardAppService_Tests : AppTestBase
    {
        ...
        public async Task Should_Get_Income_Statistics_Daily()
        {
            ...
            UsingDbContext(
                context =>
                {
                    var standardEdition = Queryable.First(context.SubscribableEditions);
                    context.SubscriptionPayments.Add(new SubscriptionPayment
                    {
                        Amount = 100,
                        Status = SubscriptionPaymentStatus.Completed,
