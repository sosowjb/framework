...
using Shouldly;
using SOSOWJB.Framework.Organizations;
using SOSOWJB.Framework.Organizations.Dto;
using Xunit;

namespace SOSOWJB.Framework.Tests.Organizations
{
    public class OrganizationUnitAppService_Tests : AppTestBase
    {
        ...
        public async Task Test_CreateOrganizationUnit()
        {
            ...
            {
                var newOu = Queryable.FirstOrDefault<OrganizationUnit>(context.OrganizationUnits, ou => ou.DisplayName == newChildName);
        ...
        public async Task Test_DeleteOrganizationUnit()
        {
            ...
            UsingDbContext(context =>
            {
                ShouldBeNullExtensions.ShouldNotBeNull(Queryable.FirstOrDefault(context.Users, u => u.Id == AbpSession.UserId.Value && u.TenantId == AbpSession.TenantId.Value));
            });
        ...
        public async Task Test_AddUsersToOrganizationUnit()
        {
            ...
            //check from database
            UsingDbContext<UserOrganizationUnit>(context => Queryable.FirstOrDefault<UserOrganizationUnit>(context.UserOrganizationUnits, uou => uou.OrganizationUnitId == ou12.Id && uou.UserId == admin.Id)).ShouldNotBeNull();
        ...
        public async Task Test_RemoveFromOrganizationUnit()
        {
            ...
            UsingDbContext<EntityEntry<UserOrganizationUnit>>(context => context.UserOrganizationUnits.Add(new UserOrganizationUnit(AbpSession.TenantId, admin.Id, ou12.Id)));
            ...
            UsingDbContext(context =>
            {
                var userOu = Queryable.FirstOrDefault<UserOrganizationUnit>(context.UserOrganizationUnits, uou => uou.OrganizationUnitId == ou12.Id && uou.UserId == admin.Id);
        ...
        {
            var organizationUnit = UsingDbContext<OrganizationUnit>(context => Queryable.FirstOrDefault<OrganizationUnit>(context.OrganizationUnits, ou => ou.DisplayName == diplayName));
        ...
        {
            var organizationUnit = UsingDbContext<OrganizationUnit>(context => Queryable.FirstOrDefault<OrganizationUnit>(context.OrganizationUnits, ou => ou.Id == id));
            organizationUnit.ShouldNotBeNull();
