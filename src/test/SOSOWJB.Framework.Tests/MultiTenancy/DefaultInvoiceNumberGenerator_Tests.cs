    ...
    public class DefaultInvoiceNumberGenerator_Tests : AppTestBase
    {
        ...
        public async Task Should_Not_Generate_Same_Invoice_Number()
        {
            ...
            for (int i = 0; i < invoiceCountToGenerate; i++)
            {
                ...
                ShouldBeStringTestExtensions.ShouldNotBeNullOrEmpty(invoiceNo);
                ShouldBeTestExtensions.ShouldBe(invoiceNo.Length, 11);//Should be YYYYMM00001
        ...
        public async Task Should_Start_Over_InvoiceNo_When_Month_Changes()
        {
            ...
            var invoiceNo = await _invoiceNumberGenerator.GetNewInvoiceNumber();
            ShouldBeStringTestExtensions.ShouldBe(invoiceNo, Clock.Now.Year + Clock.Now.Month.ToString("00") + "00001");
        }
        ...
        public async Task Should_Handle_When_Invoice_Number_Exceeds_Defined_Range()
        {
            ...
            var invoiceNo = await _invoiceNumberGenerator.GetNewInvoiceNumber();
            ShouldBeStringTestExtensions.ShouldBe(invoiceNo, Clock.Now.Year + (Clock.Now.Month).ToString("00") + "100000");
            ...
            ShouldBeStringTestExtensions.ShouldBe(invoiceNo, Clock.Now.Year + (Clock.Now.Month).ToString("00") + "100001");
        }
    }
}
