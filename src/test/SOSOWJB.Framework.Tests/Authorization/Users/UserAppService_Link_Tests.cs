...
namespace SOSOWJB.Framework.Tests.Authorization.Users
{
    public class UserAppService_Link_Tests : UserAppServiceTestBase
    {
        ...
        public async Task Should_Link_User_To_Different_Tenant_User()
        {
            ...
            //Assert
            var defaultTenantAdmin = await UsingDbContextAsync(context => EntityFrameworkQueryableExtensions.FirstOrDefaultAsync(context.Users, u => u.TenantId == AbpSession.TenantId && u.Id == AbpSession.UserId));
            var defaultTenantAccount = await _userLinkManager.GetUserAccountAsync(defaultTenantAdmin.ToUserIdentifier());

            var testUser = await UsingDbContextAsync(testTenantId, context => EntityFrameworkQueryableExtensions.FirstOrDefaultAsync(context.Users, u => u.UserName == "test"));
        ...
        public async Task Should_Link_User_To_Already_Linked_User()
        {
            ...
            //Assert
            var defaultTenantAdmin = await UsingDbContextAsync(context => EntityFrameworkQueryableExtensions.FirstOrDefaultAsync(context.Users, u => u.TenantId == AbpSession.TenantId && u.UserName == User.AdminUserName));
            var defaultTenantAdminAccount = await _userLinkManager.GetUserAccountAsync(defaultTenantAdmin.ToUserIdentifier());

            var jnash = await UsingDbContextAsync(context => EntityFrameworkQueryableExtensions.FirstOrDefaultAsync(context.Users, u => u.UserName == "jnash"));
            var jnashAccount = await _userLinkManager.GetUserAccountAsync(jnash.ToUserIdentifier());

            var testTenantUser = await UsingDbContextAsync(testTenantId, context => EntityFrameworkQueryableExtensions.FirstOrDefaultAsync(context.Users, u => u.UserName == "test"));
        ...
        private async Task<int> CreateTestTenantAndTestUser()
        {
            ...
            {
                ConnectionString = SimpleStringCipher.Instance.Encrypt("Server=localhost; Database=FrameworkTest_" + Guid.NewGuid().ToString("N") + "; Trusted_Connection=True;")
            };
        ...
        private async Task LinkUserAndTestAsync(string tenancyName)
        {
            ...
            //Assert
            var linkedUser = await UsingDbContextAsync(context => EntityFrameworkQueryableExtensions.FirstOrDefaultAsync(context.Users, u => u.UserName == "jnash"));
            var linkedUserAccount = await _userLinkManager.GetUserAccountAsync(linkedUser.ToUserIdentifier());

            var currentUser = await UsingDbContextAsync(context => EntityFrameworkQueryableExtensions.FirstOrDefaultAsync(context.Users, u => u.Id == AbpSession.UserId));
