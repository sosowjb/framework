    ...
    public class UserAppService_Update_Tests : UserAppServiceTestBase
    {
        ...
        public async Task Update_User_Basic_Tests()
        {
            ...
            await UsingDbContextAsync(async context =>
            {
                ...
                ShouldBeTestExtensions.ShouldNotBe(updatedAdminUser, null);
                updatedAdminUser.Id.ShouldBe(adminUser.Id);

                //Check some properties
                ShouldBeStringTestExtensions.ShouldBe(updatedAdminUser.EmailAddress, "admin1@abp.com");
                ShouldBeTestExtensions.ShouldBe(updatedAdminUser.TenantId, AbpSession.TenantId);
                ...
                //Check roles
                ShouldBeTestExtensions.ShouldBe(updatedAdminUser.Roles.Count, 1);
                Enumerable.Any<UserRole>(updatedAdminUser.Roles, ur => ur.RoleId == managerRole.Id).ShouldBe(true);
            });
        }
        ...
        public async Task Should_Remove_From_Role()
        {
            ...
            await UsingDbContextAsync(async context =>
            {
                var roleCount = await EntityFrameworkQueryableExtensions.CountAsync<UserRole>(context.UserRoles, ur => ur.UserId == adminUser.Id);
