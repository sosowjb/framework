...
namespace SOSOWJB.Framework.Tests.Editions
{
    public class EditionAppService_Tests : AppTestBase
    {
        ...
        public async Task Should_Create_Edition()
        {
            ...
            {
                var premiumEditon = await EntityFrameworkQueryableExtensions.FirstOrDefaultAsync<Edition>(context.Editions, e => e.DisplayName == "Premium Edition");
                premiumEditon.ShouldNotBeNull();

                if (chatFeature != null)
                {
                    var sampleFeatureValue = Queryable.FirstOrDefault<EditionFeatureSetting>(context.EditionFeatureSettings, s => s.EditionId == premiumEditon.Id && s.Name == AppFeatures.ChatFeature);
        ...
        {
            var defaultEdition = UsingDbContext<Edition>(context => Queryable.FirstOrDefault<Edition>(context.Editions, e => e.Name == EditionManager.DefaultEditionName));
            ...
            {
                defaultEdition = Queryable.FirstOrDefault<Edition>(context.Editions, e => e.Name == EditionManager.DefaultEditionName);
        ...
        public async Task Should_Delete_Edition()
        {
            ...
            UsingDbContext(context =>
            {
                ...
                Queryable.Count(context.Tenants, t => t.EditionId == defaultEdition.Id).ShouldBe(0);
            });
        }
    }
}
